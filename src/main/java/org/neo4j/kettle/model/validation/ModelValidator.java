package org.neo4j.kettle.model.validation;
...
public class ModelValidator {
  ...
  /**
   * Validate the existence of indexes and constraints in the Neo4j database before any load takes place
   *
   * @param log     The log channel to write to when there are validation errors. Validation successes are logged in Detailed
   * @param session The Neo4j session to use to validate
   * @return the number of validation errors
   */
  public int validateBeforeLoad( LogChannelInterface log, Session session ) {
    // Validate the nodes
    //
    int nrErrors = 0;
   ...
   * @return the number of validation errors.
   */
  private int validateMandatoryFields( LogChannelInterface log ) {
    int nrErrors = 0;
  ...
  /**
   * See if the specified node property is indexes.
   * If it's not, write an error to the log and return true
   *
   * @param log
   * @param node
   * @param property
   * @return the number of validation errors.
   */
  private int validateNodePropertyIndexed( LogChannelInterface log, GraphNode node, GraphProperty property, boolean unique ) {
    int nrErrors = 0;
    boolean found = false;
  ...
  private void readConstraintsData( Session session ) {
    constraintsList = session.readTransaction( new TransactionWork<List<ConstraintDetails>>() {
      @Override public List<ConstraintDetails> execute( Transaction transaction ) {
